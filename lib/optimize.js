...
export default async function optimize({ paths, lossless: isLossless, output, config, resizePercentage }) {
...
async function optimizeByType({ fileBuffer, filePath, isLossless, config, resizePercentage }) {
	const fileExtension = path.extname(filePath).toLowerCase();
			...
			return isLossless
				...
				: sharp(fileBuffer)
					.metadata((error, metadata) => {
						...
						const resize = resizePercentage ? { width: metadata.width * (resizePercentage / 100) } : undefined;
						return sharp(fileBuffer).resize(resize);
					})
