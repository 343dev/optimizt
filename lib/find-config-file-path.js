...
export async function findConfigFilePath(providedConfigPath) {
	if (providedConfigPath) {
		...
		try {
			...
			if (error.code === 'ENOENT') throw new Error(`Config file does not exist: ${resolvedPath}`, { cause: error });
			throw new Error(`Cannot inspect configuration file ${resolvedPath}: ${error.message}`, { cause: error });
		}
		if (!stat.isFile()) throw new Error(`Config path does not refer to a file: ${resolvedPath}`);
		return resolvedPath;
	}
