...
export default async function convert({
	...
	const totalPaths = inputFilePaths.length;

	if (!totalPaths) {
		return;
	}

	log(`Converting ${totalPaths} ${getPlural(totalPaths, 'image', 'images')} (${isLossless ? 'lossless' : 'lossy'})...`);
	if (isLossless) {
	...
	const tasks = inputFilePaths.reduce((accumulator, filePath) => {
