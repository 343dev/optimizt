...
Run `npm run verify:parity`; its pre-script builds the native reference in that
container before comparing it with WebAssembly. To build directly inside the
container, invoke `npm run build:native`. For release builds, the script verifies
Linux, an x86-64 GNU compiler target, and `gcc -dumpfullversion -dumpversion`
before compiling. Sanitizer verification may
