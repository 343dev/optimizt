...
`npm run build` downloads the exact upstream Sharp tarball pinned by `packages/optimizt-sharp/package.json`, verifies its integrity, generates the wrapper, and copies both codec runtimes into the ignored `packages/optimizt/vendor` directory. Only `@343dev/optimizt` is published.
